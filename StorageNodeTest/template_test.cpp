...
TEST(Template, AddTemplate) {
	std::string input = "c:6|txid:1234567890|k:0123456789|tid:9876543210|tm:1944631469190|cmds:1 " + templateId + " " + encode(tempPostDef);
...
TEST(Template, fetchTemplate) {
	std::string input = "c:11|txid:1234567890|k:0123456789|tid:9876543210|tm:1944631469190|id:" + templateId;
	json response = getWSClient()->get(input);
	EXPECT_EQ(response["txid"].get<std::string>(), "1234567890");
	EXPECT_EQ(response["data"].get<std::string>(), tempPostDef);
	EXPECT_TRUE(true);
}
...
TEST(Template, updateTemplate) {
	std::string input = "c:6|txid:1234567890|k:0123456789|tid:9876543210|tm:1944631469190|cmds:4 " + templateId + " " + encode(tempPutDef);
	json response = getWSClient()->send(input);
	EXPECT_EQ(response["txid"].get<std::string>(), "1234567890");
...
TEST(Template, fetchUpdatedTemplate) {
	...
	json response = getWSClient()->get(input);
	EXPECT_EQ(response["txid"].get<std::string>(), "1234567890");
	EXPECT_EQ(response["data"].get<std::string>(), tempPutDef);
	EXPECT_TRUE(true);
}
...
TEST(Template, deleteTemplate) {
	std::string input = "c:6|txid:1234567890|k:0123456789|tid:9876543210|tm:1944631469190|cmds:7 " + templateId;
	json response = getWSClient()->send(input);
	EXPECT_EQ(response["txid"].get<std::string>(), "1234567890");
