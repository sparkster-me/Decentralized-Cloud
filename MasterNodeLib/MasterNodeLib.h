...
typedef std::unordered_map<std::string, Transaction> TransactionsMap; // Mapping of txId..Transaction
typedef std::unordered_map<long, Block> BlocksMap; // Mapping of blockId..Block
...
extern SubscriptionAttributes* master21; // The channel that the 21 master nodes will listen on
extern SubscriptionAttributes* globalMasterNodeChannel; // All master nodes listen on a common channel. This struct defines the channel.
extern SubscriptionAttributes* IndividualChannel; // The individual master node's channel, defined by the master node's public key.
...
bool processCodePacket(const std::string& tenantId, const std::string& code, const std::string& inputs, const std::string& signature, const std::string& publicKey);
...
// Sends the code from the given transaction off to a verifier. If tx.isComputeNode is true, then com is set to 1, else it is set to 0. Sets computeNodeId if isComputeNode is true.
...
bool inVotingTime();
// Updates the timestamp at which voting begins. This is the string all master nodes will sign
